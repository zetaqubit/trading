...
class DailyDfTest(unittest.TestCase):

  def test_load_df_one_ticker(self):
    df = daily_df.load_df_one_ticker('GOOG')
    self.assertEqual(
        list(df.columns),
  ...
  def test_load_df_one_ticker_date_range(self):
    df = daily_df.load_df_one_ticker('GOOG', dr.DateRange('2020-01-06', '2020-01-10'))
    self.assertEqual(
        list(df.date.dt.strftime('%Y-%m-%d')),
        ['2020-01-06', '2020-01-07', '2020-01-08', '2020-01-09'])

  def test_load_df_one(self):
    df = daily_df.load_df(daily_df.Cfg(['GOOG']))
  ...
  def test_load_df_multiple(self):
    df = daily_df.load_df(daily_df.Cfg(['GOOG', 'FB']))
    cols = ['volume', 'open', 'high', 'low', 'close', 'adjclose']
    self.assertEqual(
        list(df.columns),
  ...
  def test_load_df_date_range(self):
    df = daily_df.load_df(daily_df.Cfg(['GOOG', 'FB']), dr.DateRange('2020-01-06', '2020-01-10'))
    self.assertEqual(
        list(df.date.dt.strftime('%Y-%m-%d')),
        ['2020-01-06', '2020-01-07', '2020-01-08', '2020-01-09'])
    self.assertFalse(df.isnull().any().any())
